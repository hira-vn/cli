	...
	params := &resend.SendEmailRequest{
		...
		Html: fmt.Sprintf(
			`<div style="font-family: sans-serif; max-width: 400px; margin: 0 auto;">
				...
				<p style="color: #666; font-size: 14px;">Nếu bạn không yêu cầu mã này, hãy bỏ qua email này.</p>
			</div>`, code),
	}
...
func buildInvitationParams(from, to, inviterName, workspaceName, inviteURL string) *resend.SendEmailRequest {
	...
	return &resend.SendEmailRequest{
		From:    from,
		To:      []string{to},
		Subject: fmt.Sprintf("%s mời bạn tham gia %s trên Hira", subjectInviter, subjectWorkspace),
		Html: fmt.Sprintf(
			`<div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
				<h2>Bạn được mời tham gia %s</h2>
				<p><strong>%s</strong> mời bạn cộng tác trong workspace <strong>%s</strong> trên Hira.</p>
				<p style="margin: 24px 0;">
					<a href="%s" style="display: inline-block; padding: 12px 24px; background: #000; color: #fff; text-decoration: none; border-radius: 6px; font-weight: 500;">Chấp nhận lời mời</a>
				</p>
