...
// releaseVersionRE matches the exact shape of GitHub release tags
// (`vMAJOR.MINOR.PATCH`). Anything else — bare commit SHA, dirty marker,
// `git describe` "tag-N-gSHA" output, the literal "dev" — is treated as
// a dev build by IsDevBuild.
var releaseVersionRE = regexp.MustCompile(`^v\d+\.\d+\.\d+$`)

// IsDevBuild reports whether the given version string represents a
// developer-built binary rather than an official GitHub release. Dev
// binaries should never be auto-overwritten because doing so silently
// rolls back uncommitted local work and any in-progress feature branches.
func IsDevBuild(version string) bool {
	return !releaseVersionRE.MatchString(strings.TrimSpace(version))
}
...
func FetchLatestRelease() (*GitHubRelease, error) {
	...
	req, err := http.NewRequest(http.MethodGet, "https://api.github.com/repos/hira-vn/cli/releases/latest", nil)
	if err != nil {
		return nil, err
	}
...
// UpdateViaBrew runs `brew upgrade hira-vn/tap/cli`.
// Returns the combined output and any error.
func UpdateViaBrew() (string, error) {
	cmd := exec.Command("brew", "upgrade", "hira-vn/tap/cli")
...
func UpdateViaDownload(targetVersion string) (string, error) {
	...
	// Build download URL: hira_{os}_{arch}.{tar.gz|zip}
	...
	assetName := fmt.Sprintf("hira_%s_%s.%s", runtime.GOOS, runtime.GOARCH, ext)
	downloadURL := fmt.Sprintf("https://github.com/hira-vn/cli/releases/download/%s/%s", tag, assetName)
