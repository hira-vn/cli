...
import (
	...
	"github.com/hira-vn/cli/server/internal/daemon/execenv"
	"github.com/hira-vn/cli/server/internal/daemon/repocache"
	"github.com/hira-vn/cli/server/pkg/agent"
)
...
func (d *Daemon) resolveAuth() error {
	...
	if cfg.Token == "" {
		loginHint := "'hira login'"
		if d.cfg.Profile != "" {
			loginHint = fmt.Sprintf("'hira login --profile %s'", d.cfg.Profile)
		}
...
func (d *Daemon) handlePing(ctx context.Context, rt Runtime, pingID string) {
	...
	// Same pre-flight as the task-spawn path — catches an external
	// auto-update before we attempt to exec a binary that no longer exists.
	if err := agent.EnsureExecutable(entry.Path); err != nil {
		d.client.ReportPingResult(ctx, rt.ID, pingID, map[string]any{
			"status":      "failed",
			"error":       err.Error(),
			"duration_ms": time.Since(start).Milliseconds(),
...
func (d *Daemon) handleUpdate(ctx context.Context, runtimeID string, update *PendingUpdate) {
	...
	if d.cfg.LaunchedBy == "desktop" {
		...
		d.client.ReportUpdateResult(ctx, runtimeID, update.ID, map[string]any{
			"status": "failed",
			"error":  "CLI is managed by Hira Desktop — update the Desktop app to upgrade the CLI",
		})
		return
	}

	// Dev builds run from a developer's repo checkout. Auto-updating would
	// silently overwrite their freshly-built `server/bin/hira`, rolling
	// the daemon back to whatever release happens to be on GitHub and
	// destroying any in-progress feature work. Refuse symmetrically with
	// the Desktop case so the UI surfaces a clear reason.
	if cli.IsDevBuild(d.cfg.CLIVersion) {
		d.logger.Info("refusing CLI self-update: daemon is a dev build", "runtime_id", runtimeID, "update_id", update.ID, "version", d.cfg.CLIVersion)
		d.client.ReportUpdateResult(ctx, runtimeID, update.ID, map[string]any{
			"status": "failed",
			"error":  fmt.Sprintf("running a dev build (%s) — update via `make build` from the repo root, not the UI", d.cfg.CLIVersion),
		})
		return
	}
...
// For brew installs, it keeps the symlink path (e.g. /opt/homebrew/bin/hira)
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, taskLog *slog.Logger) (TaskResult, error) {
	...
	// can call the Hira API and the local daemon (e.g. `hira repo checkout`).
	agentEnv := map[string]string{
		"HIRA_TOKEN":        d.client.Token(),
		"HIRA_SERVER_URL":   d.cfg.ServerBaseURL,
		"HIRA_DAEMON_PORT":  fmt.Sprintf("%d", d.cfg.HealthPort),
		"HIRA_WORKSPACE_ID": task.WorkspaceID,
		"HIRA_AGENT_NAME":   agentName,
		"HIRA_AGENT_ID":     task.AgentID,
		"HIRA_TASK_ID":      task.ID,
	}
	// Ensure the hira CLI is on PATH inside the agent's environment.
	// Some runtimes (e.g. Codex) run in an isolated sandbox that may not
	// inherit the daemon's PATH. Prepend the directory of the running
	// hira binary so that `hira` commands in the agent always resolve.
	...
	// Catch the "external CLI auto-updated and moved its binary" case before
	// exec emits a cryptic "not found in $PATH" error. EnsureExecutable
	// returns an actionable message naming the path that was tried.
	if err := agent.EnsureExecutable(entry.Path); err != nil {
		return TaskResult{}, fmt.Errorf("agent runtime unavailable: %w", err)
	}
...
func isBlockedEnvKey(key string) bool {
	...
	if strings.HasPrefix(upper, "HIRA_") {
		return true
	}
	switch upper {
