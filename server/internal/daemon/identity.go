...
// same machine shares one UUID stored at `~/.hira/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.hira/profiles/*/daemon.id` and returns every
...
// explicitly pins HIRA_DAEMON_ID to the hostname itself, there's nothing
