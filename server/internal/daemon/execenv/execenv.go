...
// checked out on demand by the agent via `hira repo checkout`.
package execenv
...
type PrepareParams struct {
	WorkspacesRoot string            // base path for all envs (e.g., ~/hira_workspaces)
...
type TaskContextForEnv struct {
	...
	// KnowledgeContext is pre-rendered markdown (top-K business knowledge chunks
	// matched against the issue) to be written verbatim to
	// .agent_context/knowledge_context.md. Empty means no context to inject.
...
// on demand via `hira repo checkout <url>`.
