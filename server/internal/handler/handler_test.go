...
const (
	handlerTestEmail         = "handler-test@hira.vn"
...
func TestMain(m *testing.M) {
	...
	testHandler = New(queries, pool, hub, bus, knowledgeSvc, emailSvc, nil, nil, Config{AllowSignup: true})
	testPool = pool
...
func TestSendCode(t *testing.T) {
	w := httptest.NewRecorder()
	body := map[string]string{"email": "sendcode-test@hira.vn"}
	var buf bytes.Buffer
	json.NewEncoder(&buf).Encode(body)
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(), `DELETE FROM verification_code WHERE email = $1`, "sendcode-test@hira.vn")
	})
}
...
func TestSendCodeDbError(t *testing.T) {
	...
	w := httptest.NewRecorder()
	body := map[string]string{"email": "dberror-test@hira.vn"}
	var buf bytes.Buffer
	json.NewEncoder(&buf).Encode(body)
	req := httptest.NewRequest("POST", "/auth/send-code", &buf)
	req.Header.Set("Content-Type", "application/json")
...
func TestSendCodeRateLimit(t *testing.T) {
	const email = "ratelimit-test@hira.vn"
	t.Cleanup(func() {
...
func TestVerifyCode(t *testing.T) {
	const email = "verify-test@hira.vn"
	ctx := context.Background()
...
func TestVerifyCodeWrongCode(t *testing.T) {
	const email = "wrong-code-test@hira.vn"
	ctx := context.Background()
...
func TestVerifyCodeBruteForceProtection(t *testing.T) {
	const email = "bruteforce-test@hira.vn"
	ctx := context.Background()
...
func TestVerifyCodeNewUserHasNoWorkspace(t *testing.T) {
	const email = "workspace-verify-test@hira.vn"
	ctx := context.Background()
