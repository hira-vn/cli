...
import (
	...
	"github.com/hira-vn/cli/server/internal/middleware"
	db "github.com/hira-vn/cli/server/pkg/db/generated"
	"github.com/hira-vn/cli/server/pkg/protocol"
	"github.com/hira-vn/cli/server/pkg/redact"
)
...
type DaemonRegisterRequest struct {
	...
	CLIVersion      string   `json:"cli_version"` // hira CLI version
...
func (h *Handler) ClaimTaskByRuntime(w http.ResponseWriter, r *http.Request) {
	...
	// Knowledge context: always expose the workspace digest when any doc is
	// published, even when the claim-time query yields zero hits — otherwise
	// agents in fresh workspaces never discover the KB exists. Top-K chunks
	// are appended after the digest when retrieval succeeds, and those hits
	// are recorded as citations so the UI can later show "agent đã tham
	// khảo tri thức gì" even after the workdir is garbage-collected.
	if h.KnowledgeService != nil && resp.WorkspaceID != "" {
		query := buildKnowledgeQuery(r, h, task)
		claim := h.KnowledgeService.RenderClaimContext(r.Context(), resp.WorkspaceID, query, 6)
		if claim.Markdown != "" {
			resp.KnowledgeContext = claim.Markdown
		}
		if claim.HitCount > 0 {
			h.KnowledgeService.RecordCitations(r.Context(), uuidToString(task.ID), resp.WorkspaceID, claim.Hits)
		}
		// When the workspace has a knowledge graph, auto-append the
		// hira-knowledge MCP server to the agent's mcp_config so the
		// runtime can call search_knowledge / get_doc / etc. mid-task. The
		// agent process inherits HOME, so the spawned `hira` subprocess
		// reuses the daemon's saved config for server URL + token.
		if claim.HasDocs && resp.Agent != nil {
			if merged, err := mergeKnowledgeMCPConfig(resp.Agent.McpConfig, resp.WorkspaceID); err != nil {
				slog.Warn("knowledge: merge mcp config failed", "error", err)
			} else {
		...
		slog.Info("knowledge.claim",
			...
			"has_docs", claim.HasDocs,
			"doc_count", claim.DocCount,
			"query_bytes", len(query),
			"hit_count", claim.HitCount,
			"rendered_bytes", len(claim.Markdown),
			"mcp_injected", claim.HasDocs && resp.Agent != nil,
		)
	}
...
// mergeKnowledgeMCPConfig injects the hira-knowledge MCP server entry
// into an agent's existing mcp_config JSON (or builds a fresh one). If the
// existing config is missing, malformed, or lacks an "mcpServers" map, a
// fresh envelope is produced — we never silently drop the agent's prior
// MCP servers on valid input. The returned json.RawMessage is safe to
// assign back to resp.Agent.McpConfig.
func mergeKnowledgeMCPConfig(existing json.RawMessage, workspaceID string) (json.RawMessage, error) {
	root := map[string]any{}
	if len(existing) > 0 {
		if err := json.Unmarshal(existing, &root); err != nil {
			// Preserve nothing rather than corrupt a malformed caller payload
			// — start with an empty envelope.
			root = map[string]any{}
		}
	}
...
// buildKnowledgeQuery composes a retrieval query from the task context:
// issue title + description + last few comments for normal tasks, chat
// message for chat tasks. Comments are included so issues whose description
// is a short instruction ("help with refund") still retrieve against the
// richer discussion the team already had in comments.
func buildKnowledgeQuery(r *http.Request, h *Handler, task *db.AgentTaskQueue) string {
	if task == nil {
		return ""
	}
	if task.IssueID.Valid {
		...
		// Pull the last up-to-3 comments (newest first in content order
		// hurts nothing — Search tokenizes everything regardless).
		if comments, err := h.Queries.ListComments(r.Context(), db.ListCommentsParams{
			IssueID:     issue.ID,
	...
	if task.ChatSessionID.Valid {
		msgs, err := h.Queries.ListChatMessages(r.Context(), task.ChatSessionID)
		if err != nil {
			return ""
		}
		for i := len(msgs) - 1; i >= 0; i-- {
