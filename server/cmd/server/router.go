...
import (
	...
	"github.com/hira-vn/cli/server/internal/auth"
	"github.com/hira-vn/cli/server/internal/events"
	"github.com/hira-vn/cli/server/internal/handler"
	"github.com/hira-vn/cli/server/internal/middleware"
	"github.com/hira-vn/cli/server/internal/realtime"
	"github.com/hira-vn/cli/server/internal/service"
	"github.com/hira-vn/cli/server/internal/storage"
	"github.com/hira-vn/cli/server/internal/util"
...
func NewRouter(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus) chi.Router {
	...
	knowledgeSvc := service.NewKnowledgeService(pool, bus)
	h := handler.New(queries, pool, hub, bus, knowledgeSvc, emailSvc, store, cfSigner, signupConfig)
	...
	r.Group(func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Get("/api/tasks/{taskId}/knowledge-citations", h.ListTaskKnowledgeCitations)
			...
			r.Route("/api/knowledge", func(r chi.Router) {
				r.Route("/docs", func(r chi.Router) {
					r.Get("/", h.ListKnowledgeDocs)
					r.Post("/", h.CreateKnowledgeDoc)
					r.Route("/{id}", func(r chi.Router) {
						r.Get("/", h.GetKnowledgeDoc)
						r.Put("/", h.UpdateKnowledgeDoc)
						r.Delete("/", h.DeleteKnowledgeDoc)
						r.Post("/reindex", h.ReindexKnowledgeDoc)
					})
				})
				r.Route("/entities", func(r chi.Router) {
					r.Get("/", h.ListKnowledgeEntities)
					r.Route("/{id}", func(r chi.Router) {
						r.Get("/", h.GetKnowledgeEntity)
						r.Put("/", h.UpdateKnowledgeEntity)
						r.Delete("/", h.DeleteKnowledgeEntity)
						r.Post("/merge", h.MergeKnowledgeEntity)
						r.Get("/graph", h.GetKnowledgeEntityGraph)
					})
				})
