...
import (
	...
	"github.com/hira-vn/cli/server/internal/daemon"
	logger_pkg "github.com/hira-vn/cli/server/internal/logger"
)
...
func init() {
	...
	f.String("daemon-id", "", "Unique daemon identifier (env: HIRA_DAEMON_ID)")
	f.String("device-name", "", "Human-readable device name (env: HIRA_DAEMON_DEVICE_NAME)")
	f.String("runtime-name", "", "Runtime display name (env: HIRA_AGENT_RUNTIME_NAME)")
	f.Duration("poll-interval", 0, "Task poll interval (env: HIRA_DAEMON_POLL_INTERVAL)")
	f.Duration("heartbeat-interval", 0, "Heartbeat interval (env: HIRA_DAEMON_HEARTBEAT_INTERVAL)")
	f.Duration("agent-timeout", 0, "Per-task timeout (env: HIRA_AGENT_TIMEOUT)")
	f.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: HIRA_DAEMON_MAX_CONCURRENT_TASKS)")
	...
	rf.String("daemon-id", "", "Unique daemon identifier (env: HIRA_DAEMON_ID)")
	rf.String("device-name", "", "Human-readable device name (env: HIRA_DAEMON_DEVICE_NAME)")
	rf.String("runtime-name", "", "Runtime display name (env: HIRA_AGENT_RUNTIME_NAME)")
	rf.Duration("poll-interval", 0, "Task poll interval (env: HIRA_DAEMON_POLL_INTERVAL)")
	rf.Duration("heartbeat-interval", 0, "Heartbeat interval (env: HIRA_DAEMON_HEARTBEAT_INTERVAL)")
	rf.Duration("agent-timeout", 0, "Per-task timeout (env: HIRA_AGENT_TIMEOUT)")
	rf.Int("max-concurrent-tasks", 0, "Max tasks running in parallel (env: HIRA_DAEMON_MAX_CONCURRENT_TASKS)")
...
// Empty profile → ~/.hira/, named profile → ~/.hira/profiles/<name>/.
...
func runDaemonForeground(cmd *cobra.Command) error {
	...
	serverURL := cli.FlagOrEnv(cmd, "server-url", "HIRA_SERVER_URL", "")
	if serverURL == "" {
	...
	cfg.LaunchedBy = os.Getenv("HIRA_LAUNCHED_BY")
